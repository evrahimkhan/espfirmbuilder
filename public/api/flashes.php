...
verify_csrf();rate_limit('flash-completed',30,3600);$data=body();
...
$q=db()->prepare('INSERT INTO flash_events(user_id,chip,firmware_size,manifest_verified) VALUES(?,?,?,?)');$q->execute([$user['id'],$chip,$size,!empty($data['manifest_verified'])?1:0]);
audit_event('firmware.flashed',['chip'=>$chip,'firmware_size'=>$size,'manifest_verified'=>!empty($data['manifest_verified'])]);
