...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    ...
    elseif(!empty($record['github_token'])){
        ...
        if($token){
            $verificationStarted=microtime(true);$headers=[];$body='';$ch=curl_init('https://api.github.com/user');
            ...
            $duration=(int)round((microtime(true)-$verificationStarted)*1000);
            db()->prepare('UPDATE users SET github_verified_at=NOW(),github_connection_ok=? WHERE id=?')->execute([$github['connected']?1:0,$user['id']]);
            operational_metric('github.connection.verify',$duration,$github['connected']?'success':'failure',['http_status'=>$status]);
            $record['github_verified_at']=date('Y-m-d H:i:s');$record['github_connection_ok']=$github['connected']?1:0;
        }
    ...
    json_response(['settings' => [
        ...
        'github_verified_at' => $record['github_verified_at'] ?? null,
