...
const modeButton=document.querySelector('.mode:not(.forgot)');
...
function setMode(next){
 ...
 passwordField.style.display=mode==='forgot'?'none':'';passwordField.required=mode!=='forgot';
 modeButton.hidden=mode==='reset';forgotButton.hidden=mode==='register'||mode==='reset';
 modeButton.innerHTML=mode==='register'?'Already registered? <b>Sign in</b>':'New here? <b>Create an account</b>';
 form.querySelector('button[type=submit]').textContent=mode==='forgot'?'Send reset link →':mode==='reset'?'Set new password →':'Continue →';
 document.querySelector('.auth-head h2').textContent=mode==='forgot'?'Reset your password':mode==='reset'?'Choose a new password':'Welcome to ESPForge';
}
...
form.onsubmit=async event=>{
 ...
 let action=mode==='register'?'register':mode==='forgot'?'request_password_reset':mode==='reset'?'reset_password':'login';
