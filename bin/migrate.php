...
function current_schema_matches(PDO $pdo): bool {
    $required=['users'=>['id'=>'bigint unsigned','email'=>'varchar(255)'],'repositories'=>['id'=>'bigint unsigned','full_name'=>'varchar(255)'],'builds'=>['source_commit_sha'=>'char(40)','analyzer_version'=>'varchar(40)','target_config_json'=>'longtext','workflow_sha256'=>'char(64)'],'schema_migrations'=>['migration'=>'varchar(190)','checksum'=>'char(64)']];
    foreach($required as $table=>$columns)foreach($columns as $column=>$type){$q=$pdo->prepare('SELECT COLUMN_TYPE,DATA_TYPE FROM information_schema.COLUMNS WHERE TABLE_SCHEMA=DATABASE() AND TABLE_NAME=? AND COLUMN_NAME=?');$q->execute([$table,$column]);$row=$q->fetch();if(!$row)return false;$actual=strtolower((string)$row['COLUMN_TYPE']);if($column==='target_config_json'){$actual=strtolower((string)$row['DATA_TYPE']);if(!in_array($actual,['json','longtext'],true))return false;}elseif($actual!==$type)return false;}
    return true;
}
