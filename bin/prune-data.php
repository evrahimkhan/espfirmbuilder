...
try{
    ...
    // Include legacy plaintext .json files so deployment of encrypted .cache
    // storage also removes old source-derived cache data.
    $cacheFiles=array_merge(glob(dirname(__DIR__).'/storage/cache/analysis-*.cache')?:[],glob(dirname(__DIR__).'/storage/cache/analysis-*.json')?:[]);$removed=0;$now=time();
    foreach($cacheFiles as $file)if(!is_file($file)||str_ends_with($file,'.json')||$now-(int)filemtime($file)>172800){if(@unlink($file))$removed++;}
    $cacheFiles=array_values(array_filter(glob(dirname(__DIR__).'/storage/cache/analysis-*.cache')?:[],'is_file'));usort($cacheFiles,fn($a,$b)=>(int)filemtime($b)<=>(int)filemtime($a));$total=0;
