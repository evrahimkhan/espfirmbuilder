...
while($processed<$limit){
    ...
    try{
        ...
        foreach($targets as &$target){$configJson=json_encode($target,JSON_THROW_ON_ERROR|JSON_UNESCAPED_SLASHES);$aiLibraries=[];if($key&&in_array($provider,['google','openrouter'],true)&&in_array($target['type']??'',['arduino','arduino_define'],true)){$source=$github->sourceBundle($job['full_name'],$job['source_commit_sha'],$tree['tree']??[]);$aiLibraries=(new AITargetAnalyzer($provider,$key,AppPolicy::aiModel($config,$provider)))->discoverLibraries($source,$target);$official->validateLibraries($aiLibraries);}$materialized=BuildMaterializer::materialize($github,$job['full_name'],$job['source_commit_sha'],$tree['tree']??[],$paths,$target,$targets,$aiLibraries);$digest=hash('sha256',$job['source_commit_sha']."\0".AppPolicy::ANALYZER_VERSION."\0".$configJson."\0".$materialized['workflow_sha256']);$insert->execute([uuid_v4(),$job['repo_id'],$job['source_commit_sha'],AppPolicy::ANALYZER_VERSION,substr((string)$target['id'],0,190),substr((string)$target['name'],0,120),$configJson,encrypt_secret($materialized['workflow']),$materialized['workflow_sha256'],$digest]);$q=$pdo->prepare('SELECT id,plan_uuid,plan_sha256 FROM build_plans WHERE repo_id=? AND source_commit_sha=? AND analyzer_version=? AND target_id=?');$q->execute([$job['repo_id'],$job['source_commit_sha'],AppPolicy::ANALYZER_VERSION,$target['id']]);$plan=$q->fetch();$target['plan_id']=(int)$plan['id'];$target['plan_uuid']=$plan['plan_uuid'];$target['plan_sha256']=$plan['plan_sha256'];}
