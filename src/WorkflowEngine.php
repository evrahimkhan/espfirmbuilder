...
YAML;
    ...
    private static function arduinoSteps(array $paths,string $source): string
    {
        ...
        foreach($additionalLibraries as $library)if(is_string($library)&&preg_match('/^[A-Za-z0-9][A-Za-z0-9 _.-]{0,99}@[0-9]+\.[0-9]+\.[0-9]+(?:[-+][A-Za-z0-9.-]+)?$/',$library)){$name=strtolower((string)strstr($library,'@',true));if(!isset($resolved[$name])&&!in_array($name,$gitProvided,true)){$resolved[$name]=$library;$aiLibraries[]=$library;}}
        $install=$libraries ? implode("\n",array_map(fn($lib)=>'          arduino-cli lib install '.escapeshellarg($lib),$libraries)) : '          echo "No reviewed registry libraries detected"';
        foreach($aiLibraries as $library)$install.="\n          arduino-cli lib install ".escapeshellarg($library)." || echo ".escapeshellarg('AI suggestion '.$library.' is unavailable in Arduino Library Manager; compilation will continue with verified dependencies.');
