...
// Content-Security-Policy is intentionally owned by public/.htaccess. Apache's
// `Header always set` applies the same policy to static pages and PHP APIs and
// replaces any upstream value, avoiding merged or contradictory policies.
...
function require_method(string ...$allowed): void { $method=strtoupper((string)($_SERVER['REQUEST_METHOD']??'GET'));if(!in_array($method,$allowed,true)){header('Allow: '.implode(', ',$allowed));json_response(['error'=>'Method not allowed.'],405);} }
...
function verify_csrf_value(mixed $value): void { if(!is_string($value)||!hash_equals($_SESSION['csrf']??'',$value))json_response(['error'=>'Invalid CSRF token'],419); }
function verify_csrf(): void { verify_csrf_value($_SERVER['HTTP_X_CSRF_TOKEN']??''); }
...
/** Connection-scoped MySQL advisory lock shared by every PHP host using this database. */
final class DatabaseOperationLock {
    public function __construct(private PDO $pdo,private string $name){}
    public function __destruct(){try{$q=$this->pdo->prepare('SELECT RELEASE_LOCK(?)');$q->execute([$this->name]);}catch(Throwable $error){error_log('ESPForge operation lock release failed: '.$error->getMessage());}}
}
function try_operation_lock(string $name): DatabaseOperationLock|false {
    $lockName='espforge:'.substr(hash('sha256',$name),0,54);$pdo=db();
    try{$q=$pdo->prepare('SELECT GET_LOCK(?,0)');$q->execute([$lockName]);return (int)$q->fetchColumn()===1?new DatabaseOperationLock($pdo,$lockName):false;}
    catch(Throwable $error){error_log('ESPForge operation lock acquisition failed: '.$error->getMessage());return false;}
}
...
function analysis_cache_get(string $namespace,string $key,int $ttl=86400): ?array { global $config; $directory=dirname(__DIR__).'/storage/cache';$hash=hash_hmac('sha256',$namespace."\0".$key,(string)$config['security']['encryption_key']);$path=$directory.'/analysis-'.$hash.'.cache';if(!is_file($path))return null;if(time()-(int)filemtime($path)>$ttl){@unlink($path);return null;}$raw=@file_get_contents($path);if(!is_string($raw)||strlen($raw)>3*1024*1024){@unlink($path);return null;}$json=decrypt_secret($raw);if(!is_string($json)){@unlink($path);return null;}$value=json_decode($json,true);if(!is_array($value)){@unlink($path);return null;}return $value; }
function analysis_cache_set(string $namespace,string $key,array $value): void { global $config; $directory=dirname(__DIR__).'/storage/cache';if(!is_dir($directory)&&!@mkdir($directory,0700,true)&&!is_dir($directory))return;$json=json_encode($value,JSON_UNESCAPED_SLASHES);if(!is_string($json)||strlen($json)>2*1024*1024)return;$encrypted=encrypt_secret($json);$hash=hash_hmac('sha256',$namespace."\0".$key,(string)$config['security']['encryption_key']);$path=$directory.'/analysis-'.$hash.'.cache';$temporary=@tempnam($directory,'analysis-');if($temporary===false)return;if(@file_put_contents($temporary,$encrypted,LOCK_EX)!==false){@chmod($temporary,0600);@rename($temporary,$path);}@unlink($temporary); }
...
function audit_identifier(string $value): string { global $config; return hash_hmac('sha256',strtolower(trim($value)),(string)$config['security']['encryption_key']); }
function operational_metric(string $name,?int $durationMs,string $outcome,array $metadata=[]): void { try{$q=db()->prepare('INSERT INTO operational_metrics(metric_name,duration_ms,outcome,metadata_json) VALUES(?,?,?,?)');$q->execute([substr($name,0,80),$durationMs===null?null:max(0,$durationMs),substr($outcome,0,32),$metadata?json_encode($metadata,JSON_UNESCAPED_SLASHES):null]);}catch(Throwable $error){error_log('ESPForge metric write failed: '.$error->getMessage());} }
