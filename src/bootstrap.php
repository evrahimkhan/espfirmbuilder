...
function analysis_cache_get(string $namespace,string $key,int $ttl=86400): ?array { global $config; $directory=dirname(__DIR__).'/storage/cache';$hash=hash_hmac('sha256',$namespace."\0".$key,(string)$config['security']['encryption_key']);$path=$directory.'/analysis-'.$hash.'.json';if(!is_file($path))return null;if(time()-(int)filemtime($path)>$ttl){@unlink($path);return null;}$raw=@file_get_contents($path);if(!is_string($raw)||strlen($raw)>2*1024*1024){@unlink($path);return null;}$value=json_decode($raw,true);if(!is_array($value)){@unlink($path);return null;}return $value; }
