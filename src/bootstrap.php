...
function analysis_cache_get(string $namespace,string $key,int $ttl=86400): ?array { global $config; $directory=dirname(__DIR__).'/storage/cache';$hash=hash_hmac('sha256',$namespace."\0".$key,(string)$config['security']['encryption_key']);$path=$directory.'/analysis-'.$hash.'.json';if(!is_file($path)||time()-(int)filemtime($path)>$ttl)return null;$raw=@file_get_contents($path);if(!is_string($raw)||strlen($raw)>2*1024*1024)return null;$value=json_decode($raw,true);return is_array($value)?$value:null; }
function analysis_cache_set(string $namespace,string $key,array $value): void { global $config; $directory=dirname(__DIR__).'/storage/cache';if(!is_dir($directory)&&!@mkdir($directory,0700,true)&&!is_dir($directory))return;$json=json_encode($value,JSON_UNESCAPED_SLASHES);if(!is_string($json)||strlen($json)>2*1024*1024)return;$hash=hash_hmac('sha256',$namespace."\0".$key,(string)$config['security']['encryption_key']);$path=$directory.'/analysis-'.$hash.'.json';$temporary=@tempnam($directory,'analysis-');if($temporary===false)return;if(@file_put_contents($temporary,$json,LOCK_EX)!==false){@chmod($temporary,0600);@rename($temporary,$path);}@unlink($temporary); }
